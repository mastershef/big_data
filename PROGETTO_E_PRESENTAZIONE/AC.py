...
def visualizza_topics(lda, feature_names, no_top_words):
    mydict = {"Termini piu' probabili per topic":[]}
    for topic_idx, topic in enumerate(lda.components_):
        mydict["Termini piu' probabili per topic"].append(" ".join([feature_names[i] for i in topic.argsort()[:-no_top_words - 1:-1] if feature_names[i]!="the"]))
